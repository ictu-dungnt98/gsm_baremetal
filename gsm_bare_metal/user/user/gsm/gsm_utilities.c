...
void gsm_utilities_get_sim_ccid(uint8_t *imei_buffer, uint8_t *result, uint8_t max_lenth)
{
    ...
    char *p = strstr((char *)imei_buffer, "+ICCID: ");
    if (p == NULL) {
        *result = 0;
        return;
    }
...
/*
+CUSD: 1,"84353078550. TKG: 0d, dung den 0h ngay 18/02/2020. Bam chon dang ky:1. 15K=3GB/3ngay2.
30K=7GB/7ngayHoac bam goi *098#",15 OK hoac +CUSD: 4 +CME ERROR: unknown
*/
...
void gsm_utilities_get_network_operator(char *buffer, char *nw_operator, uint8_t max_len)
{
     ...
     * AT+COPS=?
     * +COPS:
     * (2,"Viettel","Viettel","45204",7),(1,"VIETTEL","VIETTEL","45204",2),(1,"VIETTEL","VIETTEL","45204",0),
     * (1,"Vietnamobile","VNMOBILE","45205",2),(1,"VN
     * VINAPHONE","GPC","45202",2),(1,"Vietnamobile","VNMOBILE","45205",0), (1,"VN
     * Mobifone","Mobifone","45201",7),(1,"VN VINAPHONE","GPC","45202",0),(1,"VN
     * Mobifone","Mobifone","45201",0), (1,"VN VINAPHONE","GPC","45202",7),(1,"VN
     * Mobifone","Mobifone","45201",2),,(0-4),(0-2)
     *
...
#else /* dungnt98 */
    /* +COPS: 0,2,"45204",7 */
    if (strstr((char *)buffer, "45201")) {
        strcpy(nw_operator, "Mobifone");
    } else if (strstr((char *)buffer, "45202")) {
        strcpy(nw_operator, "VinaPhone");
    } else if (strstr((char *)buffer, "45204")) {
        strcpy(nw_operator, "Viettel");
    } else if (strstr((char *)buffer, "45205")) {
        strcpy(nw_operator, "Vietnamobile");
    }
#endif
}
...
bool gsm_utilities_parse_http_action_response(char *response, uint32_t *error_code,
...
bool gsm_utilities_parse_timestamp_buffer(char *resp, void *date_time)
{
    struct tm m_time_local;
    memset(&m_time_local, 0, sizeof(struct tm));

    if (strstr((char *)resp, "OK")) {
        char *p_msg = strstr((char *)resp, "+CCLK: ");
        if (p_msg == NULL)
            return false;

        p_msg = p_msg + strlen("+CCLK: ");
        p_msg = strchr(p_msg, '\"');
        if (p_msg == NULL)
            return false;
        p_msg = p_msg + 1;
        ...
        if (p_msg == NULL)
            return false;
        m_time_local.tm_year = atoi(p_msg);

        // Get month
        p_msg = strtok(NULL, "/");
        if (p_msg == NULL)
            return false;
        m_time_local.tm_mon = atoi(p_msg);

        // Get day
        p_msg = strtok(NULL, ",");
        if (p_msg == NULL)
            return false;
        m_time_local.tm_mday = atoi(p_msg);

        // Get hour
        p_msg = strtok(NULL, ":");
        if (p_msg == NULL)
            return false;
        m_time_local.tm_hour = atoi(p_msg);

        // Get minute
        p_msg = strtok(NULL, ":");
        if (p_msg == NULL)
            return false;
        m_time_local.tm_min = atoi(p_msg);

        // Get second
        p_msg = strtok(NULL, "+");
        if (p_msg == NULL)
            return false;
        m_time_local.tm_sec = atoi(p_msg);
    }

    memcpy(date_time, &m_time_local, sizeof(struct tm));
