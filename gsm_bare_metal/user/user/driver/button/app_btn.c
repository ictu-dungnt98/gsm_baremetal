...
void app_btn_scan(void* params)
{
    ...
    for (i = 0; i < m_btn_config.btn_count; i++) {
        ...
        if (m_btn_data[i] ^ m_hw_params[i].last_state) {
            if (m_btn_data[i] != m_hw_params[i].idle_level) /* press */
            {
                m_btn_press_evt |= 1 << i;
                m_btn_hold_count[i] = m_btn_config.get_tick_cb(); /* get time button was pressed */
            } else                                                /* idle - release */
            {
    ...
    /* read all button */
    for (i = 0; i < m_btn_config.btn_count; i++) {
        if (m_btn_data[i] != m_hw_params[i].idle_level) /* button still being pressed */
        {
            if (m_btn_config.get_tick_cb() - m_btn_hold_count[i] >= APP_BTN_HOLD_SO_LONG_TIME_MS) {
                if (((m_btn_hold_evt >> i) & 0x1)) /* if this button was hold before */
                {
                    if (m_btn_config.get_tick_cb() - m_btn_on_hold_count[i] >= APP_BTN_ON_HOLD_TIME_FIRE_EVENT_MS) /* hold enough time
                                                                                                                      for trigger event
                                                                                                                      hold button */
                    {
                        m_btn_on_hold_count[i] = m_btn_config.get_tick_cb();
                        m_btn_on_hold_evt |= (1 << i); /* set event hold */
                    }
                } else {
                    m_btn_on_hold_count[i] = m_btn_config.get_tick_cb();
                    m_btn_hold_evt |= (1 << i);
        ...
        if ((m_btn_press_evt >> i) & 0x01) {
            if (m_btn_last_press_time[i] == 0) /* first press after release before */
            {
                m_btn_last_press_time[i] = m_btn_config.get_tick_cb();
            }

            if (m_btn_config.get_tick_cb() - m_btn_last_press_time[i] >= APP_BTN_DOUBLE_CLICK_TIME_MS) /* timeout double click */
            {
                m_btn_press_cnt[i] = 0;
            } else {
                m_btn_press_cnt[i] += 1;
                ...
                if (evt != APP_BTN_EVT_MAX) {
                    if (NULL != m_callback_table[evt]) {
                        m_callback_table[evt](m_hw_params[i].pin, evt, m_custom_data[evt]); /* call to callback
                                                                                               double/tripple click button
                                                                                             */
                    }
                }
            }
            ...
            if (NULL != m_callback_table[APP_BTN_EVT_PRESSED]) {
                m_callback_table[APP_BTN_EVT_PRESSED](m_hw_params[i].pin, APP_BTN_EVT_PRESSED, m_custom_data[APP_BTN_EVT_PRESSED]); /* call to callback
                                                                                                                                       button press */
            }
            m_btn_press_evt &= ~(1 << i);
        }
        if ((m_btn_release_evt >> i) & 0x01) {
            if (NULL != m_callback_table[APP_BTN_EVT_RELEASED]) {
                m_callback_table[APP_BTN_EVT_RELEASED](m_hw_params[i].pin, APP_BTN_EVT_RELEASED, m_custom_data[APP_BTN_EVT_RELEASED]); /* call to callback
                                                                                                                                          button release */
            }
            m_btn_release_evt &= ~(1 << i);
        ...
        if (!((m_btn_hold_evt_exec >> i) & 0x1) && ((m_btn_hold_evt >> i) & 0x1)) {
            ...
            if (m_callback_table[APP_BTN_EVT_HOLD]) {
                m_callback_table[APP_BTN_EVT_HOLD](m_hw_params[i].pin, APP_BTN_EVT_HOLD, (void*)hold_time); /* call to callback hold button */
            }
        }
        ...
        if ((m_btn_on_hold_evt >> i) & 0x1) {
            ...
            if (m_callback_table[APP_BTN_EVT_HOLD_SO_LONG]) {
                m_callback_table[APP_BTN_EVT_HOLD_SO_LONG](m_hw_params[i].pin, APP_BTN_EVT_HOLD_SO_LONG, (void*)hold_time); /* call to callback hold so long button */
            }
